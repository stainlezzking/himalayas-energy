export const servicesMetaData = {
  title: "Our Services | EPCC, Pipeline Fabrication, Oil and Gas Exploration",
  description:
    "Explore Himalayas Energy’s full-service offering, including EPCC, pipeline fabrication, oil and gas exploration, and waste management. We provide world-class services for Nigeria and Africa’s energy sector.",
  keywords: [
    "EPCC Services Nigeria",
    "Pipeline Fabrication",
    "Oil and Gas Exploration",
    "Subsea Construction",
    "Field Development Oil and Gas",
    "Waste Management Oil Industry",
    "Onshore and Offshore Projects",
  ],
  openGraph: {
    type: "website",
    url: "https://himalayasenergyltd.com/services",
    title: "Our Services | Himalayas Energy Consults Limited",
    description:
      "Himalayas Energy offers comprehensive oil and gas services, from engineering and procurement to field development and waste management, driving Africa’s energy future.",
    images: [
      {
        url: "https://himalayasenergyltd.com/banner.png",
        width: 1200,
        height: 630,
        alt: "Himalayas Energy Oil and Gas Services",
      },
    ],
    site_name: "Himalayas Energy Consults Limited",
  },
  twitter: {
    card: "summary_large_image",
    title: "Our Services | Himalayas Energy",
    description:
      "We provide expert services in pipeline installation, exploration, field development, and waste management, helping clients meet their energy needs efficiently.",
    images: [
      {
        url: "https://himalayasenergyltd.com/banner.png",
        alt: "Oil and Gas Services by Himalayas Energy",
      },
    ],
    site: "@HimalayasEnergy",
  },
  alternates: {
    canonical: "https://himalayasenergyltd.com/services",
  },
  robots: {
    index: true,
    follow: true,
  },
};
